...
<body>
                ...
                <div class="nav-item" onclick="showTarget(${{t.idx}})">
                    ...
                    <div class="nav-item-info">
                        ...
                        <div class="nav-item-badges">
                            ...
                            ${{t.tech_count ? `<span class="nav-badge success">${{t.tech_count}} tech</span>` : ''}}
                        </div>
                    </div>
                </div>
            `).join('');
    ...
    def _generate_tech_section(self, results: Dict[str, Any]) -> str:
        ...
        for key, (label, css_class) in category_map.items():
            ...
            if techs:
                ...
                tags = " ".join([f'<span class="tech-tag {css_class}">{t}</span>' for t in techs[:15]])
                if len(techs) > 15:
                    tags += f' <span class="tech-tag service">+{len(techs) - 15} more</span>'
                categories_html += f'''
    ...
    def _generate_dns_section(self, results: Dict[str, Any]) -> str:
        ...
        records_html = ""
        for record_type, records in list(dns_records.items())[:5]:
            if records:
                tags = " ".join([f'<span class="tech-tag cdn">{r[:40]}</span>' for r in records[:4]])
                if len(records) > 4:
                    tags += f' <span class="tech-tag service">+{len(records) - 4}</span>'
                records_html += f'''
        ...
        if asn_info:
            ...
            <div style="margin-top: 0.75rem; padding-top: 0.75rem; border-top: 1px solid var(--border-color);">
                <h4 style="margin-bottom: 0.5rem; color: var(--text-primary); font-size: 0.8125rem;">ASN Information</h4>
                <div class="data-grid">
                    <div class="data-item">
            ...
            <div class="section-content">
                <div class="data-grid">
                    <div class="data-item">
                        ...
                        <div class="data-value">{", ".join(ip_addresses.get("ipv4", [])[:3]) or "N/A"}</div>
                    </div>
                    <div class="data-item">
                        <div class="data-label">IPv6 Addresses</div>
                        <div class="data-value">{", ".join(ip_addresses.get("ipv6", [])[:1]) or "N/A"}</div>
                    </div>
                </div>
